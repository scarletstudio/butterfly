...
def compute_internal_matching_metrics(
    ...
    metric_match_distribution = metric.intent_distribution_in_selected_matches(
        output.matches
    )
    (
        metric_community_distribution_1,
        metric_community_distribution_2,
    ) = metric.intent_distribution_in_community(output.users)

    return MatchingMetrics(
        n_proposed_matches_per_user=metric_output,
        distribution_in_final_set=metric_match_distribution,
