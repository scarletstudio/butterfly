...
def main():
    ...
    df_recent_matches["users"] = df_recent_matches["users"].apply(
        ast.literal_eval
    )
    df_recent_matches["release"] = df_recent_matches["release"].apply(
        from_release_tag
    )
    ...
    df_recent_matches = df_recent_matches[
        df_recent_matches["release"] < args.release
    ]
    ...
    with_retries = best_effort_minimize_repeat_matches(
        n_retries=args.matching_retries
    )
